'use strict';
/* PAPTrack — the offline service worker.
   ==========================================================================
   ...
   2. CACHE STORAGE IS ORIGIN-WIDE, NOT PER APP. Every app in the family shares
      ...
      (`fin-shell-`, `sv-shell-`, `td-shell-`, `gh-shell-`, `lot-shell-`) sit in
      ...
      is anything they couldn't read from GitHub. The supplies stay in
   ...
   The scope is this file's own directory. Widening it past `/paptrack/`
   ...
   running here, the app refuses to open it rather than letting normalizeItem()
   rebuild every item without the fields that build added — and push the loss
   back to the cloud, where the iPhone and Android apps would pick it up.
...
/* THE ALLOWLIST, and the security boundary of this file. Every entry is a file
   ...
   the supplies, not a Firestore reply, not tests.html, not the repo's own
...
/* Not on the list, and worth saying why: the tab icon and the Apple touch icon
   are base64 data URIs in the markup, so they arrive with the page and need no
   entry. `favicon.ico` is here because a browser fetches it from the site root
   on its own, and the header wears it. */
...
/* Resolved against this file's own URL, so the same list works unchanged on
   ...
   /paptrack/). Don't hard-code the directory. */
