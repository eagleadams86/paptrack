...
- **Nothing below the header is painted until the first `render()` has run — `data-booting` (2026-09-12).** The cards inside `<main>` are all built by `render()`, so `<main>` measures 0 at first paint. The browser paints the parsed body before the main script executes, so first paint was the header and then the **footer immediately under it**: the privacy line — "…sign in with Google…" — flashing at the top of the page on every refresh, before the app appeared and shoved it back down. Charles saw it in Money Map; a sweep of all eight pages on 2026-09-12 found **five** doing it (Money Map, Sprint Velocity, League Night, PAPTrack and the `claude-starter` template) and three clean (Golf Handicap, Flow Metrics, the NY calculator — clean because their cards are REAL MARKUP inside `<main>`). The fix is the arrangement the head script already uses for the theme: a thing settled a frame late is a page that visibly jumps as it opens.
...
- **`attachSnapshot()` is guarded by `!unsub` at BOTH call sites** — the tail of `startSync()` and pushNow's re-attach. The push paths of `startSync` already attach the listener inside `pushNow()`, so an unguarded attach at the tail opened a second `onSnapshot` and overwrote `unsub`, leaking a listener nothing could detach: doubled reads on every signed-in load, plus a spurious "Not syncing" toast at sign-out when the orphan erred. That exact leak shipped for a day after the 2026-08-17 re-attach fix. Pinned in tests.html: every `attachSnapshot();` call must carry the guard. **The error callback also schedules a once-a-minute retry (`relistenTimer`, 2026-09-01)**, because the push-side re-attach only helps a device that saves — one that only reads stayed cut off until its next sign-in. `attachSnapshot()` clears the timer, and so do sign-out and a profile switch (the retry would otherwise re-open the old profile's document). Same fix as Money Map's `listen()`, same day; pinned in the suite's last group.
...
- **The web shell is the app family's, not the phone app's, since 2026-08-21.** It had been a 760px column with the header inside it — a phone layout shown on a desktop — and it now matches the family: `--page-w`, read by BOTH `.wrap` and `.headbar` (they must be the same number or the brand stops lining up with the first card), a **sticky full-width header** with the row inside held to that width, and the family's one-line brand — 22px mark, name, then a muted `· Charlie's Epic Supply Tracker` behind a middle dot (the family's strapline pattern since 2026-08-21 — Golf Handicap's "Epic League Tracker", Flow Metrics' "Epic Flow Clinic"; the tab title and meta description still say what the app is for, which is what those are there to do). Consequences worth knowing before touching any of it:
  ...
  - **NEITHER GRID HERE TAKES THE FAMILY'S TILE-ROW RULE, AND THAT IS DELIBERATE (2026-08-23).** Money Map, Sprint Predictability and Flow Metrics now answer one rule in CSS: a tile row fills ONE line when the line has room for every tile and splits into EQUAL rows when it does not, with the count read by `:has(> :nth-child(N):last-child)` and the width by a container query, because their rows vary in count and `auto-fit` was leaving a lone tile stranded on a row of its own. Neither shape here is that. `.stats` is always exactly three — Clean, Replace, Reorder — so three columns is already gap-free and the min of `minmax(0, 1fr)` is doing a different job entirely (it is what stops a 320px phone scrolling sideways). `.list` is a card LIST, not a tile row: its count is unbounded, its floor is a measured number rather than a taste, and a fourth card starting a second row is how a list of things reads. If a tile row that VARIES in count is ever added here, take the family block from Sprint Predictability rather than writing a third answer.
  ...
  - **The add/edit dialog is 820px and lays its sections out two-across at ≥860px, and 1200px three-across at ≥1260px**, with the preset and name spanning both columns so the form does NOT rearrange between adding (preset shown) and editing (preset hidden). Paired sections are matched with `grid-template-rows: subgrid` over a three-row span — heading, field card, footnote — because only two of the four sections have a footnote, and a plain stretch hands the spare height to whatever is elastic (it made the Notes card 60px taller than its single input needed). The `@supports` fallback is the flex stretch, not a broken layout.
  - **Three across is 1200px because that is where a column measures 377px — the width it already has two-across at 820px** (2026-08-23). Nothing inside a section is re-tuned for the third column; there are simply three of them, and the form drops from 958px tall in an 868px window (opening already scrolled) to 786px with nothing below the fold. The number is set by Replacement's five quick-picks: 321px of buttons, so 347px of column once the card's padding and border are counted, and three of those plus the gaps and the dialog's padding is 1200. **The breakpoint is 1260 and was tried at 1150 first**, where the dialog came out 1097 and the quick-picks wrapped — the browser's own default `dialog` cap of `calc(100% - 6px - 2em)` bites before this file's `100vw - 32px` does, and a classic scrollbar takes 15px more that the media query never sees. Either the window has room for three full columns or it stays two-across; there is no part-formed middle. Three across at 820px was measured too and is what the breakpoint exists to prevent: 251px columns, wrapped quick-picks, and a date box asked to render in 107px when it cannot go under 129.
  - **The details window is the SAME 820px, sharing the rule** (2026-08-23). `#formDialog, #itemDialog` is one selector on purpose: the two windows are the same supply, and the Edit button inside the details window opens the form. At the base `dialog` width of 560px the wider form opened out from under the narrower window it was launched from, which read as the app jumping rather than as one thing being edited. If the form's width ever changes, the details window comes with it — that is what the shared selector is for, and it is what took the details window to 1200px too. **At that width it splits into two columns**: what the supply IS on the left (status, the actions, the facts) and what has HAPPENED to it on the right, which is the one part that grows. Left whole it would have been a 190-character line of prose and a history row with its date stranded 1,000px from its label — `.hist li` is `space-between`. The `.detailcols` wrapper is presentational only; `renderDetails` writes into `#itemStatus`, `#itemFacts` and `#itemHistory` by id and does not care what holds them.
  ...
  - **The header controls run sync → profile picker → Back Up → Share → theme, the family's order since 2026-08-22** (this bullet said theme → Back up → sync until then). The THEME PICKER IS ALWAYS LAST — it is the one control that changes nothing but how the page looks. Sync sits at the far LEFT of the right-aligned row on the same argument that used to put it last: it is the only control whose label changes width as you use it ("☁️ Sign In to Sync" → an account name → "⚠️ Not syncing"), and at the left end its growing pushes into empty space instead of shoving three controls sideways. The profile picker leads the fixed controls because it governs everything below it — change it and the whole list changes.
  ...
  - **`color-scheme` is the theme pack's, not this app's, since 2026-09-12 (pack rule 18).** `theme.css` emits it on every palette block — `:root`, each `[data-theme]`, BOTH halves of `auto`, and the print block — derived from each theme's `isLight` in `tokens.json`, so the pairing follows the PALETTE rather than the theme's name and sepia, a warm *light* theme, reads `light`. It is how a page tells the browser which way round it is, so browser-drawn UI follows: without it the calendar button inside `<input type="date">` is a near-black glyph on a near-black field on the dark themes, and that glyph is not restylable from CSS (number spinners, checkboxes and scrollbars have the same problem more quietly). It was the only thing this app's four per-theme blocks still carried, `--ring` and `--shadow` having both gone, so those blocks went with it. It was hand-copied into every app until then, and by that date it had been **missed on ten pages** and had drifted into two spellings of the Auto half. **Don't re-add a local block** — it is drift now, and `check_consumers.py` flags it.
...
- **The header's control row is ONE LINE THAT SCROLLS SIDEWAYS, BESIDE THE NAME, at every width (2026-09-14, family-wide).** Charles: *"should we just make them both always single line side scrollers?"* — Money Map was built first (financial-plan `b14f4ae`) and this is its header half; the page has no tab bar. `.headrow` holds `.top-actions` — still the scroller's name — and a `.rownav` of two arrows. **The row stays beside the name and scrolls there** — a correction the same evening (Money Map `2c19e01`; Charles: *"keep buttons beside the name"*). The first cut was content-sized and flex-wrap took the whole row under the name the moment it did not fit, which at 705px was an 88.5px bar with the name alone on top. Now it is Money Map's rule verbatim: `flex: 1 1 0%`, `min-width: min(15rem, 100%)`, `justify-content: flex-end` on the row. It wraps under the name only when the name would leave it under 15rem — an upright phone, a 480px window. The `.headbar` "Wraps, deliberately" note still holds for the name and the row; the row itself never wraps. Things to keep:
  - **No `justify-content: flex-end` on `.top-actions`.** It had one, and in a scroller that overflows, flex-end pushes the overflow off the LEFT edge where no scroll reaches — the sync button would have been unreachable. On `.headrow` it is safe, because the row never overflows: the scroller inside it shrinks.
  - **`margin-left: auto` is gone from `.headrow`.** It right-aligned the content-sized row; a row that grows to fill its line leaves an auto margin nothing to take, and the row's flex-end right-aligns the controls on either line.
  - **`wireScrollRow(row, nav)` is Money Map's, verbatim**: arrows shown only while the row overflows, each disabled at its own end, a press stepping 80% of the row. **It now carries the keyboard reveal**: a `focusin` listener, guarded by `:focus-visible` and measured against the row's padding edges, that scrolls a control half past either edge WHOLE into the row. The browser only scrolls a focus target that is entirely hidden, so Tab left Back Up 20.8px clipped at 480px (92.8px on a 390px phone, and the profile picker 50.2px at 320px). The listener is byte-identical in Money Map, PAPTrack and both lottery pages. The function keeps itself true — ResizeObserver, a MutationObserver (the profile picker is un-hidden after the script fills it; the sync label changes), the row's scroll, the window's resize — so nothing calls it after boot. It is called just before the boot block, outside the `data-booting` gate, which the header was never in.
  - **NOTHING THE PAGE SCROLLS TO LANDS UNDER THE STICKY BAR — the clearance is on the ROOT, and MEASURED.** `html { scroll-padding-top: calc(var(--header-h, 96px) + 8px) }`, and `trackHeaderHeight()` — called straight after `wireScrollRow`, byte-identical in both lottery pages — writes `--header-h` from the header's own box, rounded up, and a ResizeObserver keeps it true. The 8px is room for the focus ring. It covers both jobs at once:
    - **A keyboard focus** (Charles, 2026-09-14: *"fix the sticky header shift+tab issue too"*; Golf Handicap's `8935363`). The browser scrolls a focused control to the top edge of the window, which is where the sticky bar sits, so Shift+Tab back up the page parked control after control behind it. Measured with the full kit loaded, walking real Shift+Tab from the page end: **12 of 136 stops hidden at 1280×800, 11 on a 390px phone, 54 on a sideways 844×390 phone → 0 at all three.** Tab down from the top hid none before or after (a downward focus meets the bottom edge).
    - **The skip link.** It lands `<main>` 8px under the bar. `<main>` carried a `scroll-margin-top` of its own — a fixed 96px, which put `<main>` 22px under the 118px bar at 320px, and then the header + 20px for a few hours — and **it is gone on purpose**: an element's scroll-margin and the root's scroll-padding ADD, so keeping both would land the skip link 28px down. Don't put it back.
    - 96px is only the fallback for a page whose script never ran.
    - Tests (floor 130): the skip-link test reads the root's padding (header + 8px, following the header when the frame widens) and asserts `<main>` adds no margin; a new one focuses a control scrolled above the window with a plain `focus()` and requires it below the header, plus the rule and the missing `main` margin by source. **Its frame is 200px tall on purpose**: Chrome CENTRES a control a script focuses, where a real Tab lines it up with the top edge, so in the first 300px frame the centre was already below the bar and the live check passed on the broken page. In 200px it is red on the previous commit (control top 80px against an 88.5px header bottom).
  - **The arrows are for a mouse or trackpad only** — `@media (hover: none), (pointer: coarse) { .rownav { display: none !important } }`; on touch they grew Money Map's header. `tabindex="-1"` and `aria-hidden`, because Tab reaches every control and a focused one scrolls into view. Plain glyph buttons in this page's tokens (`--text-secondary`, `--surface-alt` on hover, `--radius-sm`), not the header's bordered controls.
  - **Print wraps the row** (the page's only `@media print` block); the pack already hides the header's buttons and pickers on paper.
  - **Measured with Playwright, fresh and with the full kit loaded (identical).** The first cut, against its own previous commit: the row stopped wrapping at **480px — header 130.5px → 88.5** — and on an **upright 390px phone — 138.5px → 92.5**. The beside-the-name correction, against the first cut: 1600 and 1100px unchanged (51px); **705px 88.5 → 51**, the row beside the name with 353px to scroll and the arrows up; 480px and the upright phone unchanged (88.5 and 92.5, the row under the name); **a sideways 844×390 phone 92.5 → 55**, beside the name with 220px to scroll. Real Tab walks at 705, 480, 390, 844×390 and 320px clip nothing, and every control made to straddle the edge comes in whole — the first cut clipped Back Up at 480 and 390 and the profile picker at 320, and left every straddling control clipped. The skip link, pressed from the keyboard, clears the header at all five; the first cut's 96px put `<main>` 22px under the 118px bar at 320px.
  - `tests.html` checks it live: a 360px frame (one line, arrows shown, › scrolls and turns ‹ on, no sideways page scroll) and a 1280px one (nothing overflows, no arrows, controls on the name's line), plus the touch query, the missing flex-end and the print rule by source. Three more with the correction: **at 705px the controls sit on the name's line and scroll there** (red on the first cut, 33.5px below it); **a control made to straddle the edge and focused lands wholly inside the row with its ring's room**, plus the `:focus-visible` guard by source (red with only the listener removed — Share 34.3px clipped; on the first cut the row did not overflow at 705px at all); and **the skip link from a scrolled 320px frame lands `<main>` below the header, and the clearance follows the header when the frame widens** (red on the first cut — main at 96, header bottom 114). Floor raised to 129.
...
- **Accessibility was audited properly on 2026-08-22, and what it found was invisible to axe.** axe-core reports this app clean on all four themes, phone and desktop, list and empty state, every dialog, and privacy.html — and it always did, because **axe never focuses anything, so it cannot test a focus indicator**. A Playwright harness in the scratchpad (see the family's axe-audit note) drove the keyboard instead and found four real things, all now fixed and pinned in `tests.html`:
  ...
  - **The placeholder was the browser's default** — `--text-primary` at the UA's opacity, 3.58:1 on dark, 3.84 on midnight, 4.24 on sepia. Now `--text-muted` at `opacity: 1` (5.3–5.85). The `opacity` half is load-bearing: without it the UA applies its own and undoes the colour. **Moved into the theme pack on 2026-09-05 as rule 16, in `--text-hint`** (5.05–5.33, the token defined for placeholders) — the same day's audit found three other apps had fixed only their Find box and six pages nothing, so the rule belongs to the family. This app's own rule went; the test pins the computed colour on every box on every theme instead of the source.
  ...
  - Also added: a `prefers-reduced-motion` block. Only the toast fades, but the rule was written blanket so it kept covering whatever was added later. **It moved out of this file into the theme pack on 2026-08-26** — it turned out five of the ten web pages carried one, in three different spellings, and four carried none at all, so it is pack rule 15 now and arrives with `theme.css`. Do not re-declare one here: `check_consumers.py` fails the page that does.
...
- **CONTROL BOUNDARIES ARE `--border-control` (fields: `--input-border`), NEVER `--border` or `--border-strong`** — fixed in the PACK on 2026-08-22, not here. Every bordered button in all five apps was identified by a `--border-strong` edge at 1.68–1.92:1 with a fill 1.05–1.22 off the page, which is under the 3:1 WCAG 1.4.11 asks of the thing that identifies a control. Two holes in the pack, and the second is the instructive one: `--input-border` was not gated at all and was carrying `--border-strong`'s value, and `--border-control` WAS gated at 3:1 but only against `bg` and `surface` — it failed `surfaceAlt` in all four themes while passing, and `surfaceAlt` is what a `.btn`/`.act` is FILLED with. The gate now covers all three surfaces and both tokens, and the values moved by the smallest lightness step that clears it. In this app the swap covers `.btn`, `.act`, `.filter-chip`, the header controls and the form fields; **three spots the 2026-08-22 sweep missed joined on 2026-08-31** — the toolbar's `.search` box (a field, so `--input-border`; it keeps a rule of its own, which is how it was missed), the card's `.stock button` −/+ steppers, and the form's `.quick button` preset chips (both `--border-control`; the quick chips fill with the same `--surface-alt` as the card they sit on, so the border is the only thing saying they are buttons at all). The dialog's own edge stays `--border-strong`, because a dialog is a panel. **Don't fix a weak control edge locally** — that is the drift the pack exists to prevent.
...
- **The CSV guard is the family's one rule, and it is not local taste.** `csvCell()` puts a
  leading `'` on any cell opening with one of OWASP's six leads — `=` `+` `-` `@` TAB CR —
  UNLESS the whole cell is a number (`PLAIN_NUMBER`). A supply name is free text, a share
  link and a restored backup are both ways somebody else's text reaches this browser, and a
  spreadsheet runs `=cmd|...` the moment the file opens: the same reasoning as `esc()` on the
  way to `innerHTML`, in a format with its own execution model. **Nothing this app stores is
  negative** — every count is validated at or above zero — so the carve-out changes no figure
  here today. It is carried anyway because the identical two lines are in Flow Metrics, Golf
  Handicap, Sprint Predictability, the starter and both lottery pages, and Golf's score
  differentials DO go below zero. A change to the rule belongs in all seven. The `.ics`
  exporter is the same class of boundary in a different grammar — see `escICS`/`foldICS`.
...
- **`tests.html` carries a render SMOKE WALK, and it is not a substitute for the tests
  ...
  measurement is the point: `renderDetails`, `openForm`, `renderKeepingFocus` and 68 others sat at zero. A throw in any of them
  ...
  **It writes nothing**, and that is enforced rather than intended: `__plant` replaces the supply list in memory and never calls save(); the filters and the
  search box are plain variables rather than stored preferences, so there is nothing else to
  put back. The windows are opened from the CARD'S OWN buttons rather than by `showModal`: a
  dialog opened directly is empty in exactly the way a broken one is, and the first draft of
  this test read 34 characters of static markup off one and called it drawn.
...
- Registration is guarded three ways, all load-bearing: **not in a frame** (or a
  ...
  **not under `window.papHalted`** — the halt's own flag (deliberately separate
  from the share view's `viewOnly`, which arrived later), and the check matters
  because the halt's `throw` cannot reach a separate script block — and **on
  `load`**.
...
- **The halt sets its own flag, `halted`, deliberately separate from the share
  view's `viewOnly`** — checked by `save()` (the single write path) and by the
  sync module before it initialises. (This bullet said "there is no share view
  here to reuse" until 2026-08-31 — that was true when the halt was written, but
  the share view arrived 2026-08-23. The two flags stay separate on purpose: a
  halt protects a NEWER saved copy, a shared view protects the READER's copy,
  and folding them together would let leaving a snapshot un-halt a stopped app.)
