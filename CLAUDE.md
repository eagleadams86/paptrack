...
- **Token names are the PACK'S names now.** The old app-local names map: `--page`→`--bg`, `--ink`→`--text-primary`, `--ink-2`→`--text-secondary`, `--muted`→`--text-muted`, `--hairline`→`--border`, `--accent-ink`→`--on-accent`, `--chip-bg`→`--surface-alt`, `--good`→`--ok`, `--warning`→`--warn`, `--critical`→`--err`, and each `-bg` partner. `--serious`/`--serious-bg` were defined and never used; the pack supplies them. **One token stays local and is declared after the link**: `--shadow`. **It does not exist in the pack** — `--shadow` is not a pack token at all, so this app's per-theme shadows are an ADDITION, not an override of anything, and there is nothing to diverge from. (Flow Metrics and Financial Plan add their own `--shadow` the same way.) Keep it in the app's own block after the link, which is where an app token belongs. **There was a second, `--ring`, and it went on 2026-08-22** — an invented TRANSLUCENT focus colour (0.14–0.30 alpha) that composited to 1.28–1.60:1 against its own field, where WCAG 1.4.11 asks 3:1. The pack already ships a solid `--focus-border` per theme and three siblings were already using it; this app's own skip link was too. Don't reintroduce an app-local focus colour.
...
- **The web shell is the app family's, not the phone app's, since 2026-08-21.** It had been a 760px column with the header inside it — a phone layout shown on a desktop — and it now matches the family: `--page-w`, read by BOTH `.wrap` and `.headbar` (they must be the same number or the brand stops lining up with the first card), a **sticky full-width header** with the row inside held to that width, and the family's one-line brand — 22px mark, name, then a muted `· CPAP Supply Tracker` behind a middle dot. Consequences worth knowing before touching any of it:
  ...
  - **`#itemForm .sec-card` needs `align-self: start`, and subgrid is not enough on its own.** Subgrid aligns the two sections part for part; the grid's own default `stretch` was still running underneath and filling the taller row's spare height INTO the shorter card. Spares carried **171px of empty box** under two number fields because Ordering beside it has three. `flex: none` only undoes the fallback layout's stretch — both are needed. Dead space inside a bordered card is the thing this form is not allowed to have; a gap OUTSIDE the card, as plain dialog, is fine.
  ...
  - **The theme picker's `<option>`s carry `selected` on midnight, and the sun is `☀` not `☀️`** (2026-08-21). Without `selected` the browser pre-selects the FIRST option, so the row painted "☾ Dark" over a midnight page until `$('themeSel').value = ...` ran at the foot of the file — and the header paints long before that. The emoji-presentation sun is drawn from the colour font: a different weight and baseline from the `☾` and `✦` beside it, so the row read as three different sets of glyphs. Every sibling app already followed both rules; `tests.html` pins the four labels exactly and asserts `defaultSelected` on midnight.
...
- **THREE containers get focused as a mechanism in this app, and each needs its own answer.** `<main tabindex="-1">` (the skip link's landing) and `<dialog>` take `outline: none` outright — nobody is about to operate them. **The supply CARD is the conditional one**: `renderKeepingFocus()` focuses it when the button just pressed has removed itself (Mark Cleaned and Mark Replaced both do), and from the KEYBOARD that ring is the only thing telling you where you landed, so it must stay. From a finger it must not — a whole card lighting up after a tap is noise. **Chromium and desktop WebKit both work that out and leave `:focus-visible` unmatched after a tap; real iOS Safari does not**, so the app says which it was instead of trusting the heuristic: every call site passes `e.detail > 0` (0 when a button was activated from the keyboard, non-zero for mouse or finger), and the pointer case adds `.quiet-focus`, cleared on `blur`. `tests.html` pins that EVERY call site passes it — one that forgets rings the card on a phone, and no desktop will ever show you.
...
- **The one accessibility item left open is NOT this app's to fix.** Every secondary (bordered) button in the family — `.btn`, `.act`, the chips, and the same controls in Golf Handicap, Flow Metrics and Money Map — is identified by a `--border-strong` edge measuring **1.68–1.92:1** with a fill only 1.05–1.22:1 different from the page behind it. On the strict reading of WCAG 1.4.11 that is under the 3:1 a control boundary wants; on the reading in WCAG's own understanding document these controls are identifiable from their text and position and need no 3:1 edge. Either way it is a `tokens.json` question with a `check_contrast.py` gate in front of it, and the gate does not currently check border-against-background pairs at all. **Don't "fix" it here** — a stronger border in one app is exactly the drift the pack exists to prevent.
