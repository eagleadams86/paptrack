...
- **The header's control row is ONE LINE THAT SCROLLS SIDEWAYS, at every width (2026-09-14, family-wide).** Charles: *"should we just make them both always single line side scrollers?"* — Money Map was built first (financial-plan `b14f4ae`) and this is its header half; the page has no tab bar. `.headrow` (`flex: 0 1 auto`, and it now carries the `margin-left: auto` that `.top-actions` used to) holds `.top-actions` — still the scroller's name — and a `.rownav` of two arrows. Content-sized, the row sits beside the name while it fits, and flex-wrap takes the whole row onto a line of its own when it does not, where it shrinks and scrolls rather than wrapping. The `.headbar` "Wraps, deliberately" note still holds for the name and the row; the row itself no longer wraps. Things to keep:
  - **No `justify-content: flex-end` on `.top-actions`.** It had one, and in a scroller that overflows, flex-end pushes the overflow off the LEFT edge where no scroll reaches — the sync button would have been unreachable. The auto margins right-align the row while it fits.
  - **`wireScrollRow(row, nav)` is Money Map's, verbatim**: arrows shown only while the row overflows, each disabled at its own end, a press stepping 80% of the row. It keeps itself true — ResizeObserver, a MutationObserver (the profile picker is un-hidden after the script fills it; the sync label changes), the row's scroll, the window's resize — so nothing calls it after boot. It is called just before the boot block, outside the `data-booting` gate, which the header was never in.
  ...
  - **Measured against the previous commit** with Playwright, fresh and with the full kit loaded: 1600px, 1100px, 705px and a sideways 844×390 phone are pixel-identical. The old row wrapped at **480px — header 130.5px → 88.5, arrows shown** — and on an **upright 390px phone — 138.5px → 92.5**, no arrows. `main { scroll-margin-top: 96px }` stays: it now clears every measured header, where the old two-line phone bar was 42px taller than it.
  - `tests.html` checks it live in a 360px frame (one line, arrows shown, › scrolls and turns ‹ on, no sideways page scroll) and a 1280px one (nothing overflows, no arrows, controls on the name's line), plus the touch query, the missing flex-end and the print rule by source. Confirmed red on the old page.
