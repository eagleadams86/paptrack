...
- **An info dot on each of the three counts** — the "i" beside 🧼 Clean, ↻ Replace and 📦 Reorder opens a short window explaining what that count actually counts: that zero days left is due rather than due tomorrow, that the cleaning clock starts at the later of the last wash and the last replacement, that a reorder is driven by the spares threshold alone and has nothing to do with the replacement date, and how paused, snoozed and on-order differ. There are three dots and no more, on purpose — everywhere else this app already says its working out loud on the card ("Replace every 3 months · Last replaced yesterday · Replace by Nov 20 · 89 days left"), and those three counts are the only place a number is the result of a rule you cannot see. The dot and the window are the same in every app in the family. *(Web only for now — the iOS and Android apps have no help windows.)*
...
- **The theme picker is written out at its final size, with Auto pre-selected** (Midnight until 2026-08-22, when the family default moved). The header paints long before the script at the foot of the page runs, so an option list built by script made the row read "☾ Dark" over a midnight page for a moment on every load. The sun is `☀`, the plain text character, not the emoji-presentation `☀️` — the colour-font variant is a different weight and baseline from the `☾` and `✦` beside it, and the row read as three different sets of glyphs. Both fixed 2026-08-21 and pinned in `tests.html`; every sibling app follows the same two rules
- **Fits the screen it's on** — the page is the same width as Money Map, the widest in the family (nothing here stretches to it: the extra width is spent on another column of cards, not a wider card), the supply cards flow into two or three columns as the window allows, and cards on the same row are the same height so a row has a straight bottom edge. On a phone it's the single column it always was. The header is a bar across the top that **stays put as you scroll**, so the theme picker, **Back up** and sync are always a click away rather than somewhere above the first supply. **Its controls are always one line, beside the name**: when they don't all fit, the row keeps its place next to the name and scrolls sideways there — flick it, or with a mouse or trackpad press the **‹ ›** arrows that appear at the end of the row whenever a control is off an edge. Only on a phone held upright, or a window about that narrow, where the name leaves too little room, does the row take a line of its own under the name. Tabbing onto a control that is half past the edge scrolls it fully into view. They used to wrap to two lines on a phone, which made the header 138px tall; it is 92px now, and a 705px window or a phone on its side keeps a one-line 51–55px bar. **Skip to content**, and every control you reach with Tab or Shift+Tab, always lands just below the bar — never hidden behind it — whatever height the bar is at that moment
...
- **Share a read-only link** — pick which supplies ride, and the link opens a snapshot anyone can read with no
  sign-in and no way to change it. The supplies travel *inside* the link: nothing is uploaded and no copy is
  kept, which also means a link cannot be withdrawn once sent. **Your notes, what each one costs and where you
  order it, and the full cleaning history are left out unless you tick them** — the schedule, what is due and
  how many spares are on hand are what a link is for. Opening one hides every control that would write, and
  `save()` refuses outright. The family's window, the same in all six apps
...
- **Search & filters** — live search plus three chips: 🧼 Clean due, ↻ Replace due (due today or overdue), 📦 Reorder due; tapping the active chip again clears it back to showing everything. **⌘K / Ctrl-K puts the caret in the search box** and selects what's already there, so typing replaces it. Money Map, Sprint Predictability, Flow Metrics and Golf Handicap open a **Find** window on those same keys — this app has searched from the page since the toolbar existed, and a window over the top of it would be a second way to do one thing, so the keys reach the box instead. Nothing happens while a dialog is open (the add/edit form is exactly where pulling focus away would be a trap) or before there is anything to search
- **More than one person on one app** — two people on one CPAP household kept their supplies apart by keeping two Google accounts, which also means two sign-ins, two backups and two phones set up. Each person is now a profile with their own supplies, countdowns and spares; the theme, the reminders and the backup file are shared. The picker is **always in the header, even for a household of one** — its last entry, *Manage people…*, is the only door to the window where people are added, renamed and removed, so hiding the picker below two people (which shipped for an hour) hid the feature itself. The phones hide their menu on that rule but keep a People row in Settings; the web has no Settings screen, so one always-present control is the honest answer. Removing someone deletes their supplies and says so first
...
- **CSV for a spreadsheet** — two one-way exports in the Back up dialog, beside the JSON that is the actual backup: one row per supply, and one row per history event. Every cell starting with `=`, `+`, `-` or `@` — or with a tab or a carriage return in front of one — is defused with a leading apostrophe, because a spreadsheet reads those as formulas and a supply name is free text; a cell that is simply a number is left as a number, so the day counts and the spares column stay arithmetic
...
- **Accessible, and audited rather than assumed** — every control has a name, and the repeated card buttons say which supply they belong to ("Delete — Mask"), so a screen reader never just reads "Delete, Delete, Delete". The stat tiles' emoji rows are announced as sentences ("2 supplies due for cleaning: Mask, Tubing"), keyboard focus stays on the button you pressed when the list re-renders, and all four themes meet WCAG AA contrast (4.5:1) down to the smallest labels. Audited on 2026-08-22 with axe-core **and** a keyboard-driven pass, on every theme, phone and desktop, in every dialog: everything is reachable by Tab in the order it is read, dialogs take focus, close on Esc and hand it back to the button that opened them, the page reflows at 320px without scrolling sideways, and motion is dropped for anyone whose system asks for none. Form fields wear the family's colours too — a surface fill on the pack's input border, not the page's own near-black. **Every control draws the same focus ring** — 2px in the theme's own focus colour, 4.5:1 or better against whatever is behind it, on the button and the chip and the link, not just the text fields
...
- **Buttons are the family's** — a bordered `.btn` with a filled `.primary` variant drawn from the pack's `--btn-bg`/`--btn-text`, the same rule Sprint Predictability, Flow Metrics and Money Map carry. Until 2026-08-22 this was the one app whose primary button was an accent-purple pill, so two of them open side by side didn't look like one product. No colour changed: the primary pair was already in the palette and unused here, and `--accent` keeps its other jobs. **+ Add Supply** wears that button plain, with nothing of its own — until 2026-08-22 it was a pill fixed to the bottom-right corner of the window, a phone pattern that on a desktop hung over the list as the one control in the app that wasn't part of the page. It now sits in the toolbar row between the search box and the filters, the way Golf Handicap's **+ Add Round** sits above the rounds it adds to
- **Notes sits with the name, and it is a proper text box** — three lines, so a note you can actually read rather than one truncated mid-word. The add/edit form is a two-column grid above 860px: the preset picker takes the first row on its own, then Name & Notes beside Cleaning, with the two cards on any row ending at the same depth
- **The row loses a column when the status pills would have to wrap** — the grid's minimum card width is measured, not chosen: 660px is the narrowest card that keeps "Replace by Sep 5, 2026 · 14 days left" on one line. So the list goes three across on a very wide screen, two from about 1360px, and one below that, and a date pill never breaks over two lines except on a phone. Three columns is the ceiling however wide the window gets — a card holds a 2x2 block of tiles, and past three across those tiles are narrower than the words in them
- **The toolbar is one uniform row** — search box, **+ Add Supply**, the *Filters:* caption, its three chips and *Mark All Due Items Clean*, every one of them a 40px control on a single line from 720px up. The caption used to read *Filter — tap to toggle* and sit *above* its chips, which made the filter block two lines tall and set the height of the whole row: the search box stretched to 57px to match it while the chips stayed at 33px. The add button is the reference — it is the family's button and its size is not this app's to choose, so everything else is measured against it. Below 720px the row stacks, full-width, as it always did
- **4 themes, and Auto by default** — dropdown in the header, listed alphabetically (Auto, Dark, Light, Midnight, Sepia), shared with every other app in this family via the unified theme pack; choice saved in `localStorage` and applied before first paint. With nothing saved the app follows your own phone or computer — Light on a light setting, Midnight on a dark one — and changes with it while the page is open, no reload needed. Midnight is still the base palette, which is what Auto means by "dark". Forest, Solarized and Synthwave were retired in August 2026, and Slate was renamed Dark — a saved Slate preference carries over automatically
...
**And a smoke walk, because everything else is a pure function.** Pinning `normalizeItem`, the schedule maths and the `.ics` generation leaves the largest part of the file — the render layer — never executed at all, so a throw inside a card, a filter or a dialog would ship green. A coverage run on 2026-08-27 measured exactly that: `renderDetails`, `openForm`, `renderKeepingFocus` and 68 others sat at zero. The walk plants a full kit in a second, full-size frame, looks through each of the three filters, opens a supply's details and its edit form **from the card's own buttons** (a window opened by `showModal` alone is empty in exactly the way a broken one is — the first draft of this test read 34 characters of static markup off a window that had never opened and called that "drawn"), and fails if the frame throws or a window comes back with nothing in it. Verified by breaking `renderDetails` on purpose. It writes nothing: the plant hook replaces the list in memory rather than saving.
